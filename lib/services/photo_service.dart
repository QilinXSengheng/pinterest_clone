...
class PhotoService {
  static Future<Either<String, List<AllPhotoModel>>> getPhotos() async {
    try {
      Response response = await Dio().get(Endpoints.getPhotos, options: Options(headers: {'x-api-key': Endpoints.apiKey, 'Authorization': 'Bearer ${Endpoints.token}'}));
  ...
  static Future<Either<String, List<Result>>> searchPhotos({String? search, int? page}) async {
    try {
      Response response = await Dio()
          .get('${Endpoints.searchPhotos}?query=$search&page=$page&per_page=50&order_by=ASC', options: Options(headers: {'x-api-key': Endpoints.apiKey, 'Authorization': 'Bearer ${Endpoints.token}'}));
      Log.w(response.statusCode.toString());
      if (response.statusCode == 200 || response.statusCode == 201) {
        List<Result> photos = [];
